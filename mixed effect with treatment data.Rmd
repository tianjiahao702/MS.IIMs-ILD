...
library(lmerTest) # for calculating p-values
```
...
# Create a new column called "treatment" that contains the treatment each ID took
Treat <- Treat %>%
  mutate(treatmentV1V2 = case_when(
    ...
    TRUE ~ "No Treatment" # Default value if none of the treatments are taken
  ))
```
...
# Assume your dataset is stored in a data frame called "df"
# Make sure the necessary variables are appropriately coded as factors or numerics
# For example, assuming the "treatment" variable is categorical with 10 classes:
# Convert tumor_region to a factor variable
qlf22$REGION <- as.factor(qlf22$REGION)

# Create dummy variables for tumor_region
...
# Get confidence intervals of fixed effects
#confint(model)
