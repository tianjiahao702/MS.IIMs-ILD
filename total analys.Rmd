...
```{r}
# Assuming your data is stored in a dataframe called 'your_data' and the first column is 'first_column' and the second column is 'second_column'
...
```{r}
#sur_df = t_df11[!is.na(t_df11$Death), ]
...
#model1 = coxph(Surv(time_mon, Death) ~ low_decay + stable + low_growth + 
...
#Insufficient data: If there are very few events (such as deaths in your case) in some of the categories of your predictor, the model might not have enough information to estimate the hazard ratio.
...
long_data1 = long_data1[!is.na(long_data1$Death), ]

long_data1$selectedFit = ifelse(long_data1$selectedFit == "2 evals not 20% diff", "stable", long_data1$selectedFit)
```
...
custom_palette <- c("gx" = "firebrick3", "dx" = "forestgreen", "stable" = "goldenrod1", "gd" = "steelblue1", "gdphi" = "purple1")  # specify your color palette
...
ggplot(long_data1, aes(x = date, y = QLFCAD, group = name, color = selectedFit)) +
  geom_line() +
  geom_point() +
...
custom_palette <- c("#00AFBB", "#E7B800", "#FC4E07", "black", "red")  # specify your color palette
...
ggplot(long_data1, aes(x = date, y = QLFCAD, group = name, color = treatment)) +
...
```{R}
water_df = t_df11[!(t_df11$selectedFit %in% c("not fit")), ]
...
waterfall_plot = ggplot(water_df, aes(x = reorder(name, change), y = change)) +
  ...
  scale_shape_manual(values = c("Stable: whose overall QLF score change less than 20%" = 5, "Yoyo group (convex): patients with both growth and decay rate" = 8)) +
  scale_color_manual(values = c("Stable: whose overall QLF score change less than 20%" = "black", "Yoyo group (convex): patients with both growth and decay rate" = "black")
  ) +
  labs(
    title = "Waterfall Plot",
    x = "ID",
  ...
  theme_minimal() +
  theme_grey(base_size = 17.5) +
  #theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme(axis.text = element_text(size = 9.5),  # Increase size of side labels
        legend.position = "right")# Rotate x-axis labels by 45 degrees
  #geom_text(data = data.frame(x = c(10, 40), y = c(-15, 15), label = c("dx", "gx")), aes(x, y, label = label), inherit.aes = FALSE, vjust = 0, hjust = 0, size = 10, color = "black")
