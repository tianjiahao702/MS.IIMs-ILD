...
```{r}
gdrate11 = function(input, pval, plots) {
  ...
  plotgdX = function(input1, i) {
    ...
    graphics::plot(f ~ time, data = jdta, frame = FALSE, col = "red", cex = 1.3, cex.axis = 1.4,
                   cex.lab = 1.6, pch = 19, xlab = "Days", ylab = "QLF Score (log)", main = tit)
  ...
  aggPE = function(pEst) {
    ...
    wherenone = data.frame(matrix(NaN, nrow = 1, ncol = 6, byrow = T))
    wherenone
    
    # gs
    out = subset(out3, out3$parameter %in% gnames)
    if (dim(out)[1] > 0) {
      outg = data.frame(unique(out[, colkep]))
      colnames(outg) = colsg
      outg
    } else {
      outg = wherenone
    ...
    if (dim(out)[1] > 0) {
      outd = data.frame(unique(out[, colkep]))
      colnames(outd) = colsd
      outd
    } else {
      outd = wherenone
    ...
    if (dim(out)[1] > 0) {
      outphi = data.frame(unique(out[, colkep]))
      colnames(outphi) = colsp
      outphi
    } else {
      outphi = wherenone
      colnames(outphi) = colsp
    ...
    out9 = out8[, c("name", "N", "type", "selectedFit", "finalg", "finald", "finalphi")]
  ...
  generateresults = function() {
    ...
    if (ip$cont == 0) {
      ...
      resultsanalyzable = function() {
        ...
        allconv0 = checkconv2()
        allconv = allconv0[(allconv0$selected == allconv0$fit | allconv0$selected == "not fit"), ]
        ...
        setex = function() {
          ...
          naempty = function() {
            dnone = data.frame(cbind(name = NaN, calcfinal = NA, N = NaN))
